...
# Module-level cache: domain name -> live Chroma object.
# Populated on the first retrieve() call for each domain, then used directly
# for all subsequent calls — bypassing stores.retrieve() entirely so the
# package never gets a chance to reconstruct VectorStoreInfo and reload.
_chroma_cache: Dict[str, object] = {}


def _process_results(results, k: int) -> Tuple[str, float]:
    """Shared result processing: sort, truncate to k, return text + best score."""
    if not results:
        return "", 0.0
    sorted_results = sorted(results, key=lambda t: t[1], reverse=True)
    top = sorted_results[:k]
    best_score = top[0][1]
    chunks = [doc.page_content for doc, _ in top]
    return "\n\n---\n\n".join(chunks), best_score
...
def retrieve_context(
    ...
    First call per domain: uses stores.retrieve() which triggers the lazy
    Chroma load, then caches the live Chroma object for all future calls.

    Subsequent calls: queries Chroma directly — no VectorStoreInfo rebuild,
    no config traversal, no disk reload. One Chroma instance per domain,
    held in memory for the entire process lifetime.
    ...
    """
    try:
        if domain not in _chroma_cache:
            # First call: let the package do its full load cycle
            results = stores.retrieve(domain_name=domain, query=query)

            # Immediately grab the now-populated Chroma object and cache it
            for domain_cfg in stores.vs_config.domains.values():
                for vs_info in domain_cfg.vector_stores:
                    if vs_info.loaded_object is not None:
                        _chroma_cache[domain] = vs_info.loaded_object
                        break

            return _process_results(results, k)

        else:
            # Subsequent calls: query Chroma directly, skip stores.retrieve()
            chroma = _chroma_cache[domain]
...
async def main():
    ...
    # The store is lazy-loaded — retrieve_context() will trigger the first
    # load and cache the Chroma object directly for all subsequent calls.
    # We do one warm-up retrieve here just to verify the store has content.
    print(f"  Domains: {stores.domains}")
    for _domain in stores.domains:
        _test = stores.retrieve(domain_name=_domain, query="neuron")
        print(f"  [{_domain}] loaded: {len(_test)} chunks returned for test query")
        if not _test:
            print(f"  WARNING: domain '{_domain}' returned no results — store may be empty!")

    # Silence NeuroML-AI logger now that the initial load is done.
    # The package attaches StreamHandlers during load, so clear them after.
    for _ln in ("NeuroML-AI", "RAG", "Vector_Stores", "neuroml_ai_utils", "gen_rag"):
        _lg = logging.getLogger(_ln)
        _lg.handlers.clear()
        _lg.setLevel(logging.WARNING)
        _lg.propagate = False
    print("  NeuroML-AI debug logging suppressed")
